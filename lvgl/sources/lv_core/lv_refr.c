...
static void (*onScreenRefreshed_cb)(void*, uint32_t); /* Can be used for switch between 2 LCD buffers (to prevent flickering)*/
...
void lv_refr_set_onScreenRefreshed_cb(void (*cb)(void*, uint32_t))
{
  onScreenRefreshed_cb = cb;
}
...
static void lv_refr_task(void * param)
{
    ...
    if (refr_done != false)
    {
      if (onScreenRefreshed_cb != NULL)
      {
        onScreenRefreshed_cb((void*) inv_buf, inv_buf_p);
      }
    }
    memset(inv_buf, 0, sizeof(inv_buf));
    inv_buf_p = 0;
